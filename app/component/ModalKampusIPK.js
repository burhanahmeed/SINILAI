...
import {View, Text, StyleSheet, TouchableOpacity, Alert, Picker, Vibration} from 'react-native';
...
export default class ModalKampusIPK extends Component{
    ...
    componentWillMount(){
        const ipkstring = this.props.itemCallback;
        if(ipkstring!=null){
            const arrayipk = ipkstring.split('');
    ...
    _simpanTarget(){
        firebaseApp.auth().onAuthStateChanged((user)=>{
            firebaseApp.database().ref('nilai/'+user.uid).update({
                targetIPK:this.state.amount_1+'.'+this.state.amount_2+this.state.amount_3
            }).catch((err)=>{
                alert(err);
            })
        })
    ...
    render(){
        ...
        return(        
                <View style={styles.modalContent}>
                    <View style={{}}>
                        <Text style={{fontWeight:'bold', color:'#1F3A93', fontSize:20}}>Target IPK</Text>      
